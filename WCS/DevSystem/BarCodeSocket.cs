...
namespace WCS.DevSystem
{
    ...
    public class BarCodeSocket
    {
        ...
        public BarCodeSocket(New_Main_Form mainFrm)
        {
            ...
            while (!closing)
            {
                ...
                try
                {
                    ...
                    if (SocketConnectFlag && socket_client.Available > 0)// && socket_client.Available >= cig_length
                    {
                        ...
                        if (k > 0)
                        {
                            str_receivedata = oldData + ASCIIEncoding.ASCII.GetString(by_data);
                            li_start_pos = str_receivedata.IndexOf("{", 0);
                            li_end_pos = str_receivedata.IndexOf("}", 0);
                            while (li_start_pos >= 0 && li_end_pos >= 0)
                            {
                                barcode = str_receivedata.Substring(li_start_pos + 1, li_end_pos - li_start_pos - 1);
                                if (WorkStatusRun )
                                {
                                   ...
                                   // string strSql = "update td_wcs_barcode set BARCODE  = '" + barcode.Substring(2) + "' where SCNNER_NO = '" + barcode.Substring(0,2) + "'";
                                    ...
                                    pt.Value=barcode.Substring(2);

                                      MySqlParameter ptno=new MySqlParameter("@scnnerno",MySqlDbType.VarChar,10);  
                                      ptno.Value= barcode.Substring(0,2);
                                      MySqlConnection dbConn2;
                                      dbConn2 = new MySqlConnection(mainFrm.dbConnectionString);
                                      dbConn2.Open();
                                      MySqlCommand mc = new MySqlCommand(strSql, dbConn2);
