...
namespace WCS
{
    public partial class New_Main_Form : Form
    {
        ...
        private Thread cmdDeviceThread2;
        private Thread cmdDeviceThread3;
        private Thread cmdDeviceThread4;
        ...
        public bool cmdDeviceThreadStatus2 = false;
        public bool cmdDeviceThreadStatus3= false;
        public bool cmdDeviceThreadStatus4 = false;
        ...
        /// 可以下命令的设备
        /// </summary>
        public Dictionary<string, BESDevice> cmdDevices2 = new Dictionary<string, BESDevice>();
        /// <summary>
        /// 可以下命令的设备
        /// </summary>
        public Dictionary<string, BESDevice> cmdDevices3 = new Dictionary<string, BESDevice>();
        /// <summary>
        /// 可以下命令的设备
        /// </summary>
        public Dictionary<string, BESDevice> cmdDevices4 = new Dictionary<string, BESDevice>();
        ...
        public static Thread ScannerThread3 = null;//扫描线程1
       ...
       // public static Thread ScannerThread = null;//扫描线程1
        ...
        public string dbConnectionStringWms = "Server='192.168.0.3';UserId=hmzdx;Password=hmzdx@2016;Database=wmsx;Allow User Variables=True";
      //  public string dbConnectionStringWms = "Server=localhost;UserId=root;Password=123321;Database=zshm_wcs;Allow User Variables=True";
        ...
        public string dbConnectionString = "Server=192.168.0.3;UserId=hmzdx;Password=hmzdx@2016;Database=zshm_wcs;Allow User Variables=True";
      //  public string dbConnectionString = "Server=localhost;UserId=root;Password=111111;Database=zshm_wcs;Allow User Variables=True";
        ...
        private void New_Main_Form_Load(object sender, EventArgs e)
        {
            ...
            //2
            cmdDeviceThread2 = new Thread(new ThreadStart(taskautorun.CmdDeviceAutoRun2));
            if (!cmdDeviceThread2.IsAlive)  //启动任务管理进程；
                cmdDeviceThread2.Start();
            //3
            cmdDeviceThread3 = new Thread(new ThreadStart(taskautorun.CmdDeviceAutoRun3));
            if (!cmdDeviceThread3.IsAlive)  //启动任务管理进程；
                cmdDeviceThread3.Start();
            //4
            cmdDeviceThread4 = new Thread(new ThreadStart(taskautorun.CmdDeviceAutoRun4));
            if (!cmdDeviceThread4.IsAlive)  //启动任务管理进程；
                cmdDeviceThread4.Start();
            //画连接线
        ...
        private void btnStrat_Click(object sender, EventArgs e)
        {
            try
            {
                if (dbIsConned && plcIsOK && deviceInited)
                {
                    ...
                    if (ScannerThread3 == null)
                    {
                        ScannerThread3 = new Thread(new ThreadStart(barCodeSocket3.Scanning));
                        ScannerThread3.IsBackground = true;
                        if (!ScannerThread3.IsAlive)
                        {
                            ScannerThread3.Start();
                        }
                        barCodeSocket3.WorkStatusRun = true;
                    }
                    workModel = 1;
        ...
        private void btnInitDev_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds_dev = new DataSet();
            //初始化命令设备
            ds_dev = MySqlHelper.ExecuteDataset(dbConn, "select * from td_device_dic where command_db like 'DB%' and device_id between '1001' and '1077' order by device_id");
            ...
            ds_dev = MySqlHelper.ExecuteDataset(dbConn, "select * from td_device_dic where command_db like 'DB%' and device_id between '11019' and '11046' order by device_id");

            if (ds_dev.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_dev.Tables[0].Rows)
                {
                    BESDevice device2 = new BESDevice(this, row, cmdDeviceOpcServer);
                    cmdDevices2.Add(row.ItemArray[0].ToString(), device2);//将设备添加到设备字典中
                    // frm.Text = string.Format("正在初始化设备{0}接口，请耐心等候......", device.deviceName);
                    //临时注销
                    device2.BindToPLC();
                    Thread.Sleep(600);
                    this.tabPage1.Controls.Add(device2.pic);
                    device2.pic.BringToFront();
                    Application.DoEvents();
                }
            }
            ds_dev = MySqlHelper.ExecuteDataset(dbConn, "select * from td_device_dic where command_db like 'DB%' and device_id between '11048' and '11067' order by device_id");

            if (ds_dev.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_dev.Tables[0].Rows)
                {
                    BESDevice device3 = new BESDevice(this, row, cmdDeviceOpcServer);
                    cmdDevices3.Add(row.ItemArray[0].ToString(), device3);//将设备添加到设备字典中
                    // frm.Text = string.Format("正在初始化设备{0}接口，请耐心等候......", device.deviceName);
                    //临时注销
                    device3.BindToPLC();
                    Thread.Sleep(600);
                    this.tabPage1.Controls.Add(device3.pic);
                    device3.pic.BringToFront();
                    Application.DoEvents();
                }
            }

            ds_dev = MySqlHelper.ExecuteDataset(dbConn, "select * from td_device_dic where command_db like 'DB%' and device_id between '11077' and '11077' order by device_id");

            if (ds_dev.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow row in ds_dev.Tables[0].Rows)
                {
                    BESDevice device4 = new BESDevice(this, row, cmdDeviceOpcServer);
                    cmdDevices4.Add(row.ItemArray[0].ToString(), device4);//将设备添加到设备字典中
                    // frm.Text = string.Format("正在初始化设备{0}接口，请耐心等候......", device.deviceName);
                    //临时注销
                    device4.BindToPLC();
                    Thread.Sleep(600);
                    this.tabPage1.Controls.Add(device4.pic);
                    device4.pic.BringToFront();
                    Application.DoEvents();
                }
            }
            deviceInited = true;
        ...
        {
            //调用
            string url = "http://192.168.0.3/wms/electronAutoController/getIdxConveyor?ctCode=2011004197";
            ...
            ForSetckk frmSetBufferSize = new ForSetckk(this);
            frmSetBufferSize.Show();
        }
